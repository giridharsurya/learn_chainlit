...
def get_agents():
    ...
    email_agent = ChatCompletionAgent(
        name="email_agent",
        instructions="You are an email agent that writes a clear and professional email",
        kernel=email_kernel,
        description="An agent that drafts concise and well-structured email"
    )
...
async def agent_response_callback(message: ChatMessageContent) -> None:
    ...
    if message.content:
        await cl.Message(content=f"agent_response_callback> {message.name}: {message.content}").send()
...
async def human_response_function() -> ChatMessageContent:
    ...
    user_input = await cl.AskUserMessage(content="aaaaaaaaaaa",timeout=300).send()
    
    # user_input = await cl.user_session.get_input(timeout=300)
    return ChatMessageContent(role=AuthorRole.USER, content=user_input['output'])

agents,handoffs = get_agents()


async def get_agent_response(user_input):
    handoff_orchestration = cl.user_session.get("orchestrator")
